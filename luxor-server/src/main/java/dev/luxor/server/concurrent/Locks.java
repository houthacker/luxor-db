...
public final class Locks {
    ...
    /**
     * Acquires exclusive ownership of all given locks while avoiding deadlocks. After this method has been executed,
     * either all or no locks are owned by the calling thread.<br />
     ...
     */
    public static void exclusiveLock(ReentrantReadWriteLock... locks) {
        if (locks.length == 0) {
            return;
        }
    ...
    /**
     * Releases exclusive ownership of all provided locks. Any lock that is not exclusively owned by the
     * calling thread is left untouched.
     *
     * @param locks The list of locks of which to release exclusive ownership.
     */
    public static void exclusiveUnlock(ReentrantReadWriteLock... locks) {
